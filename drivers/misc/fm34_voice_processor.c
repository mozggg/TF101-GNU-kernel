#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/device.h>
#include <linux/miscdevice.h>
#include <linux/fs.h>
#include <linux/i2c.h>
#include <linux/slab.h>
#include <linux/gpio.h>
#include <linux/delay.h>
#include <linux/i2c/fm34_voice_processor.h>
#include <asm/ioctl.h>

struct fm34_data {
	struct list_head		list;
	struct delayed_work		init_work;
	struct i2c_client		*client;
	struct miscdevice		misc_dev;
	bool					configured;
	bool					enabled;
	int						gpio_reset;
	int						gpio_en;
};

static LIST_HEAD(fm34_list);
static int fm34_count = 0;

static uint8_t enable_cmd[] = { 0xC0, 0xFC, 0xF3, 0x3B, 0x23, 0x00, 0x00, 0x00 };
static uint8_t bypass_cmd[] = { 0xC0, 0xFC, 0xF3, 0x3B, 0x23, 0x00, 0x00, 0x04 };

u8 config_table[] = {
    
};

static void fm34_reset(struct fm34_data *data)
{
	gpio_set_value(data->gpio_reset, 0);
	msleep(10);
	gpio_set_value(data->gpio_reset, 1);
}

static int fm34_send(struct fm34_data *data, uint8_t *buf, size_t sz)
{
	int ret;

	gpio_set_value(data->gpio_en, 1);
	msleep(10);
	ret = i2c_master_send(data->client, buf, sz);
	msleep(5);
	gpio_set_value(data->gpio_en, 0);

	return ret < 0;
}

static int fm34_configure(struct fm34_data *data)
{
	struct i2c_msg msg;
	uint8_t cmd;
	int ret;

	if (data->configured)
		return 0;

	fm34_reset(data);
	msleep(100);
	gpio_set_value(data->gpio_en, 1);
	msleep(20);

	cmd = 0xC0;
	msg.addr = data->client->addr;
	msg.flags = 0;
	msg.len = 1;
	msg.buf = &cmd;

	ret = i2c_transfer(data->client->adapter, &msg, 1);
	if (ret < 0) {
		dev_err(&data->client->dev, "Failed to access device\n");
		goto out;
	}

	ret = i2c_master_send(data->client, config_table, sizeof(config_table));
	if (ret != sizeof(config_table)) {
		dev_err(&data->client->dev, "Failed to configure device\n");
		goto out;
	}

	data->configured = true;
	ret = 0;
out:
	gpio_set_value(data->gpio_en, 0);
	return ret;
}

static int fm34_set_enable(struct fm34_data *data, bool en)
{
	if (fm34_configure(data))
		return -EIO;

	if (en)
		return fm34_send(data, enable_cmd, sizeof(enable_cmd));

	return fm34_send(data, bypass_cmd, sizeof(bypass_cmd));
}

static int fm34_open(struct inode *inode, struct file *filp)
{
	struct fm34_data *data;
	int minor;

	minor = MINOR(filp->f_dentry->d_inode->i_rdev);
	filp->private_data = NULL;

	list_for_each_entry(data, &fm34_list, list) {
		if (data->misc_dev.minor == minor) {
			filp->private_data = data;
			break;
		}
	}

	if (!filp->private_data)
		return -ENODEV;

	return 0;
}

static int fm34_release(struct inode *inode, struct file *filp)
{
	filp->private_data = NULL;
	return 0;
}

static long fm34_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct fm34_data *data = filp->private_data;

	switch (cmd) {
	case FM34_IOCTL_SENABLE:
		return fm34_set_enable(data, !!arg);

	default:
		dev_err(&data->client->dev, "Invalid ioctl %u\n", cmd);
		return -ENOTTY;
	}
}

struct file_operations fm34_fops = {
	.owner			= THIS_MODULE,
	.unlocked_ioctl	= fm34_ioctl,
	.open			= fm34_open,
	.release		= fm34_release,
};

static void fm34_init_work(struct work_struct *work)
{
	struct fm34_data *data = container_of(work, struct fm34_data, init_work.work);

	dev_info(&data->client->dev, "Performing initialization\n");
	fm34_set_enable(data, false);
}

static int fm34_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	struct fm34_data *data;
	struct fm34_platform_data *pdata;
	char *name;
	int err;

	pdata = (struct fm34_platform_data *)client->dev.platform_data;
	if (!pdata) {
		dev_err(&client->dev, "No platform data\n");
		err = -EINVAL;
		goto exit;
	}

	if (!pdata->gpio_reset || !pdata->gpio_en) {
		dev_err(&client->dev, "GPIOs not specified\n");
		err = -EINVAL;
		goto exit;
	}

	data = kzalloc(sizeof(*data), GFP_KERNEL);
	if (!data) {
		dev_err(&client->dev, "Failed to alloc data\n");
		err = -ENOMEM;
		goto exit;
	}

	name = kzalloc(10, GFP_KERNEL);
	if (!name) {
		dev_err(&client->dev, "Failed to alloc name\n");
		err = -ENOMEM;
		goto exit_free;
	}
	snprintf(name, 10, "fm34_%d", fm34_count++);

	data->misc_dev.name = name;
	data->misc_dev.minor = MISC_DYNAMIC_MINOR;
	data->misc_dev.fops = &fm34_fops;
	err = misc_register(&data->misc_dev);
	if (err) {
		dev_err(&client->dev, "Failed to register misc device\n");
		goto exit_free_name;
	}

	data->gpio_reset = pdata->gpio_reset;
	data->gpio_en = pdata->gpio_en;

	i2c_set_clientdata(client, data);
	data->client = client;

	list_add(&data->list, &fm34_list);

	/* bypass the FM34, without delaying boot */
	INIT_DELAYED_WORK(&data->init_work, fm34_init_work);
	schedule_delayed_work(&data->init_work, msecs_to_jiffies(2000));

	return 0;

exit_free_name:
	kfree(data->misc_dev.name);
exit_free:
	kfree(data);
exit:
	return err;
}

static int fm34_remove(struct i2c_client *client)
{
	struct fm34_data *data = i2c_get_clientdata(client);

	list_del(&data->list);
	misc_deregister(&data->misc_dev);
	kfree(data->misc_dev.name);
	kfree(data);

	return 0;
}

static int fm34_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct fm34_data *data = i2c_get_clientdata(client);

	gpio_set_value(data->gpio_en, 0);

	return 0;
}
static int fm34_resume(struct i2c_client *client)
{
	return 0;
}

static const struct i2c_device_id fm34_id[] = {
	{ "fm34", 0 },
	{}
};

static struct i2c_driver fm34_driver = {
	.driver = {
		.name	= "fm34",
	},
	.id_table	= fm34_id,
	.probe		= fm34_probe,
	.remove		= fm34_remove,
	.suspend	= fm34_suspend,
	.resume		= fm34_resume,
};

static int __init fm34_init(void)
{
	return i2c_add_driver(&fm34_driver);
}

static void __exit fm34_exit(void)
{
	i2c_del_driver(&fm34_driver);
}

module_init(fm34_init);
module_exit(fm34_exit);
