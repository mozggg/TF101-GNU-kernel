...
#define OV5640_TOTAL_WIDTH	3200
#define OV5640_TOTAL_HEIGHT	2000
...
static struct regval_list ov5640_mcu_firmware[] = {
{0x3000,0x20},
{0x8000,0x02},
{0x8001,0x0b},
{0x8002,0x61},
...
static int ov5640_set_resolution(struct i2c_client *client)
{
	int ret;
	u8 start_x_high = ((OV5640_SENSOR_SIZE_X - OV5640_WIDTH) / 2) >> 8;
	u8 start_x_low  = ((OV5640_SENSOR_SIZE_X - OV5640_WIDTH) / 2) & 0xff;
	u8 start_y_high = ((OV5640_SENSOR_SIZE_Y - OV5640_HEIGHT) / 2) >> 8;
	u8 start_y_low  = ((OV5640_SENSOR_SIZE_Y - OV5640_HEIGHT) / 2) & 0xff;

	u8 width_high	= OV5640_WIDTH  >> 8;
	u8 width_low	= OV5640_WIDTH  & 0xff;
	u8 height_high	= OV5640_HEIGHT >> 8;
	u8 height_low	= OV5640_HEIGHT & 0xff;

	u8 total_width_high  = OV5640_TOTAL_WIDTH  >> 8;
	u8 total_width_low   = OV5640_TOTAL_WIDTH  & 0xff;
	u8 total_height_high = OV5640_TOTAL_HEIGHT >> 8;
	u8 total_height_low  = OV5640_TOTAL_HEIGHT & 0xff;
			  ...
			  struct v4l2_mbus_framefmt *mf)
{
	...
	mf->width	= OV5640_WIDTH;
	mf->height	= OV5640_HEIGHT;
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	ov5640_write_array(client, ov5640_default_regs_init);
	ov5640_write_array(client, ov5640_mcu_firmware);
	ov5640_set_resolution(client);
	ov5640_write_array(client, ov5640_default_regs_finalise);
...
static int ov5640_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	...
	rect->top	= 0;
	rect->left	= 0;
	rect->width	= OV5640_WIDTH;
	rect->height	= OV5640_HEIGHT;
...
static int ov5640_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	a->bounds.left			= 0;
	a->bounds.top			= 0;
	a->bounds.width			= OV5640_WIDTH;
	a->bounds.height		= OV5640_HEIGHT;
	a->defrect			= a->bounds;
	a->type				= V4L2_BUF_TYPE_VIDEO_CAPTURE;
 ...
 */
static unsigned long soc_ov5640_query_bus_param(struct soc_camera_device *icd)
{
	return 0;
}

static int soc_ov5640_set_bus_param(struct soc_camera_device *icd,
				 unsigned long flags)
{
	return -EINVAL;
}

static struct soc_camera_ops soc_ov5640_ops = {
	.query_bus_param	= soc_ov5640_query_bus_param,
	.set_bus_param		= soc_ov5640_set_bus_param,
};
